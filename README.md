...
_Note: Two outputs generated by this app (MpileupInformativeSNPInput.txt and the InputVariantsforphasing.txt) are required inputs to run our [Single_Cell_Somatic_Mutation_Caller](https://github.com/ShainLab/Single_Cell_Somatic_Mutation_Caller) app._

## What does this app do?
1.	<ins>Initial filtering</ins> : Processes FreeBayes variant calls to identify heterozygous SNPs. Filters for SNPs present in the 1000 Genomes database (MAF ≥ 1%) and having minor allele frequency between 30-70% (heterozygous).  
...
3.	<ins>Refinement</ins> : Further filters SNPs based on actual read data; only variants with MAF between 40-60% with read depth ≥ 10 are kept. These are the "Informative SNPs".  
...
- FBtxt - A text file containing FreeBayes output in tab-delimited format with variant information
